...
class OpsDatabaseService {
  ...
  List<DeclinedEmergencyModel> _declinedEmergencyListFromSnapshot(QuerySnapshot snapshot){
  ...
  List<OngoingEmergencyModel> _onGoingEmergencyListFromSnapshot(QuerySnapshot snapshot){
    return snapshot.documents.map((doc){
  ...
  Stream<List<AvailableMfrs>> get availableMfrStream {
    return availableMfrs.where('isActive', isEqualTo: true).where('isOccupied', isEqualTo: false).snapshots().map(_availableMfrsListFromSnapshot);
  }
