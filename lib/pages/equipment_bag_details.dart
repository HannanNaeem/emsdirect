...
class _bagDetailsState extends State<BagDetails> {
    ...
    _bagDetailsState(String _Name, String Name) {
      _name = Name;
      var index = 1;
      _bagData = Firestore.instance.collection("EquipmentBags").document(_Name).get().then(
          (doc) {
            for ( var i in names ) {
              data.add(doc[i]);
              index = index + 1;
                            ...
                            Padding(
                                ...
                                child: TextFormField(
                                    ...
                                    validator: (String value) {
                                      if (int.parse(value) < 0) return 'Quantity must be greater than zero!';

                                      if (double.parse(value) - int.parse(value) != 0) return 'Quantity must be an integer!';
                                    },
